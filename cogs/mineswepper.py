...
class minesweeper:
    ...
    async def minesweeper(self, ctx, columns = None, rows = None, bombs = None):
        ...
        if columns < 1 or rows < 1 or bombs < 1:
            await ctx.send("The provided numbers can't be zero or negative...\n**Try again!**")
            return
        ...
        embed = discord.Embed(title='Minesweeper', color=0xC0C0C0)
        embed.add_field(name='Total Spaces:', value=columns * rows, inline=True)
        embed.add_field(name='Bombs:', value=bombs, inline=True)
